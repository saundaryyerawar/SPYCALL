...
class Audio_File_Analyze_Window(QWidget):
    ...
    def Recognize_multiple_files(self):
        index = 0
        no_of_files = len(filename)
        while no_of_files != 0:
            self.Recognize_file_Audio(filename[index])
            index = index + 1
            no_of_files = no_of_files - 1

    def Recognize_file_Audio(self, AUDIO_FILE):
        print("\nfile_name : " + Path(AUDIO_FILE).stem)
        #self.status_Label.setText(AUDIO_FILE)
        r = sr.Recognizer()
    ...
    def classify(self,text):
        ...
        if(y_pred[0]==1):
            print("Threatful Audio")
        else:
            print("Not Threatful Audio")
    ...
    def __init__(self, parent):
        ...
        self.Select_TextFile_Button.setText("Select Audio File")
